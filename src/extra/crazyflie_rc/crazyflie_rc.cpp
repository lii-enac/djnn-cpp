...
namespace djnn {
	...
	public:
	  ...
	  virtual void info(const string& msg) override { std::cerr << msg << __FL__; }
	  virtual void warning(const string& msg) override { std::cerr << msg << __FL__; }
	  virtual void error(const string& msg) override { std::cerr << msg << __FL__; }
	};

	CrazyflieRC::CrazyflieRC(ParentProcess* parent, const string& name, const string& uri)
	: FatProcess(name),
	_uri(this, "uri", uri),
