...
#define GET_CHILD(type, parent, path) auto * path = dynamic_cast<type*>(parent->find_child(#path)); if (!path) { fprintf (stderr, "ERROR - there is NO " #type " \"%s\" child in process \"%s\" - %s:%s:%d\n\n", #path, #parent, __FUNCTION__,__FILE__,__LINE__);  exit(-1); }
...
#define GET_CHILD_VAR2(varname, type, parent, path) varname = dynamic_cast<djnn::type*>(parent->find_child(#path)); if (!varname) { fprintf (stderr, "ERROR - there is NO " #type " \"%s\" child in process \"%s\" - %s:%s:%d\n\n", #path, #parent, __FUNCTION__,__FILE__,__LINE__);  exit(-1); }
...
#define GET_CHILD_VALUE(varname, type, parent, path) auto * child_##varname = dynamic_cast<djnn::Abstract##type##Property*>(parent->find_child(#path)); if (!child_##varname) { fprintf (stderr, "ERROR - there is NO " #type " \"%s\" child in process \"%s\" - %s:%s:%d\n\n", #path, #parent, __FUNCTION__,__FILE__,__LINE__);  exit(-1); }; const auto& varname = child_##varname->get_value ();
#define GET_CHILD_VALUE2(varname, type, parent, path) if (auto * child_##parent = dynamic_cast<djnn::Abstract##type##Property*>(parent->find_child(#path))) {varname = child_##parent->get_value ();} else { fprintf (stderr, "ERROR - there is NO " #type " \"%s\" child in process \"%s\" - %s:%s:%d\n\n", #path, #parent, __FUNCTION__,__FILE__,__LINE__);  exit(-1); }
