...
#define GET_CHILD(varname, type, parent, name) auto * varname = dynamic_cast<djnn::type*>(parent->find_child(name)); if (!varname) { fprintf (stderr, "no " #type " \"%s\" child in process %s:%s:%d\n", name, __FUNCTION__,__FILE__,__LINE__);  exit(-1); }
