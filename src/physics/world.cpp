...
void
World::add_phy_object (PhyObj* p)
{
    _phy_objs.push_back (p);
}
void
World::remove_phy_object (PhyObj* p)
{
    _phy_objs.erase (djnnstl::remove (_phy_objs.begin (), _phy_objs.end (), p), _phy_objs.end ());
}
...
#define phyattr(x)                                       \
    if (name == #x)                                      \
        _##x = dynamic_cast<DoublePropertyProxy*> (res); \
    else
    phyattr (x)
        phyattr (y)
            phyattr (z)
