...
{
  public:
    DashArray (CoreProcess* parent, const string& name) : AbstractStyle (parent, name) { finalize_construction (parent, name); }
    DashArray (CoreProcess* parent, const string& name, const vector<double>& patterns) : AbstractStyle (parent, name), _dash_array (patterns) { finalize_construction (parent, name); }
    DashArray (CoreProcess* parent, const string& name, double patterns[], size_t pat_len) : AbstractStyle (parent, name), _dash_array (vector<double> (patterns, patterns + pat_len)) { finalize_construction (parent, name); }
...
{
  public:
    NoDashArray (CoreProcess* parent, const string& name) : AbstractStyle (parent, name) { finalize_construction (parent, name); }
    virtual ~NoDashArray () {}
    void         impl_activate () override { AbstractStyle::impl_activate (); }
    void         impl_deactivate () override { AbstractStyle::impl_deactivate (); }
    void         draw () override;
