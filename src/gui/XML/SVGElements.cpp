...
static map<string, djn_XMLTagHandler> handlers = {
...
{
    map<string, djn_XMLTagHandler>::iterator it;
...
StartPoly (const char** attrs, FatProcess* current)
{
    ...
    return holder ? holder : djn_PolyArgs.e ? djn_PolyArgs.e
                                            : current;
}
...
StartPath (const char** attrs, FatProcess* current)
{
    ...
    return holder ? holder : djn_PathArgs.e ? djn_PathArgs.e
                                            : current;
}
...
static int
parseGradientTransform (FatProcess** e, char* v)
{
    int                 success = 1;
...
StartRadialGradient (const char** attrs, FatProcess* current)
{
    ...
    djn_GradientArgs.spread    = DJN_PAD_FILL;
    djn_GradientArgs.coords    = DJN_LOCAL_COORDS;
    djn_GradientArgs.inherited = 1 << djn_GradientCoords | 1 << djn_GradientSpread | 1 << djn_GradientStopPoints | 1 << djn_GradientCx | 1 << djn_GradientCy | 1 << djn_GradientR | 1 << djn_GradientFx | 1 << djn_GradientFy;
    ...
    if (djn_GradientArgs.ancestor) {
        ...
        for (i = djn_GradientCoords; i <= djn_GradientFy; ++i) {
            if (djn_GradientArgs.inherited & (1 << i)) {
                ...
                if (strcmp (djn_Inherited[i], "stops") == 0)
                    gradient->bstops = false;
                if (strcmp (djn_Inherited[i], "transforms") == 0)
                    gradient->btransforms = false;
                if (strcmp (djn_Inherited[i], "spread") == 0)
                    gradient->bspread = false;
                if (strcmp (djn_Inherited[i], "coords") == 0)
                    gradient->bcoords = false;
            }
        }
        gradient->update ();
    }
