...
CairoGLSDLWindow::CairoGLSDLWindow (djnn::Window* win, const string& title, double x, double y, double w, double h)
      ...
      _my_cairo_surface (nullptr)
{
    ...
    if (!glad_inited) {
        ...
        if (glad_status)
            std::cerr << "glad could not open opengl" << __FL__;
