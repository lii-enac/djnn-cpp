...
namespace djnn
{
  ...
  AbstractHomography::find_child_impl (const string& name)
  {
    ...
    if (name.find ("leftTranslateBy") != string::npos) {
      init_leftTranslateBy();
      return AbstractGObj::find_child(name);
    } else
    if (name.find ("rightScaleBy") != string::npos) {
      init_rightScaleBy();
      return AbstractGObj::find_child(name);
    } else
    if (name.find ("leftScaleBy") != string::npos) {
      init_leftScaleBy();
      return AbstractGObj::find_child(name);
    } else
    if (name.find ("rightRotateBy") != string::npos) {
      init_rightRotateBy();
      return AbstractGObj::find_child(name);
    } else
    if (name.find ("leftRotateBy") != string::npos) {
      init_leftRotateBy();
      return AbstractGObj::find_child(name);
    } else
    if (name.find ("rightSkewXBy") != string::npos) {
      init_rightSkewXBy();
      return AbstractGObj::find_child(name);
    } else
    if (name.find ("leftSkewXBy") != string::npos) {
      init_leftSkewXBy();
      return AbstractGObj::find_child(name);
    } else
    if (name.find ("rightSkewYBy") != string::npos) {
      init_rightSkewYBy();
      return AbstractGObj::find_child(name);
    } else
    if (name.find ("leftSkewYBy") != string::npos) {
      init_leftSkewYBy();
      return AbstractGObj::find_child(name);
    } else
    return nullptr;
