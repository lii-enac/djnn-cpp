...
Touch::set_touch_local_coords (PickUI* p, double x, double y, bool is_move)
{
    ...
    double      loc_y = h->raw_props.m21 * x + h->raw_props.m22 * y + h->raw_props.m23 + h->raw_props.m24 - s->origin_x ()->get_value ();
...
AbstractGShape::set_mouse_local_coords (double x, double y, bool is_move)
{
    ...
    double      loc_x = h->raw_props.m11 * x + h->raw_props.m12 * y + h->raw_props.m13 + h->raw_props.m14 - _origin_x->get_value ();
    double      loc_y = h->raw_props.m21 * x + h->raw_props.m22 * y + h->raw_props.m23 + h->raw_props.m24 - _origin_y->get_value ();
