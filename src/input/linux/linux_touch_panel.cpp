...
namespace djnn {
  ...
  LinuxTouch::LinuxTouch (unsigned int fieldmap) : FatProcess ("LinuxTouch")
  {
  ...
  LinuxTouchPanel::LinuxTouchPanel (ParentProcess* parent, const djnn::string& name, const struct libevdev *dev) : LinuxDevice (parent, name, TOUCH_PANEL)
  {
  ...
  LinuxTouchPanel::~LinuxTouchPanel ()
  {
    ...
    for (vector<LinuxTouch*>::iterator it = _v_touches.begin () ; it != _v_touches.end(); ++it)
      delete *it;
    _v_touches.clear ();
