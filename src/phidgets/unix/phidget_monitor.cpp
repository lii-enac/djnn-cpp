...
namespace djnn
{
  ...
  on_attach_handler (PhidgetManagerHandle man, void *ctx, PhidgetHandle channel)
  {
    ...
    switch (id)
      ...
      case PHIDID_1010_1013_1018_1019:
        {
          IFaceKit888 *dev = (IFaceKit888*) InterfaceKits->find_component (std::to_string (deviceLabel));
          if (dev == nullptr) {
  ...
  IFaceKit888::IFaceKit888 (Process *parent, const string &name) :
      Process (n)
  {
  ...
  VoltageInputChannel::VoltageInputChannel (Process *p, const string& n) :
      Process (n)
  {
